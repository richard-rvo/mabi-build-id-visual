/* Protótipo isolado: nenhum serviço, IA, conta, persistência ou envio externo. */
'use strict';
...
function resetConversation() {
  ...
  messages.innerHTML = `<section class="chat-welcome"><div class="welcome-art">${icon('mabi')}</div><h2>O que cabe<br>no seu dia hoje?</h2><p>Pode me contar com suas palavras.<br>Ou comece por uma ideia.</p><div class="starter-stack"><button class="starter" data-prompt="Estou sem ideias para a próxima refeição.">${icon('chef')}<span><strong>Estou sem ideias</strong><small>Vamos partir do que tem em casa.</small></span>${icon('arrow')}</button><button class="starter" data-prompt="Minha rotina está corrida. Como me organizar?">${icon('clock')}<span><strong>Minha rotina está corrida</strong><small>Uma coisa de cada vez.</small></span>${icon('arrow')}</button></div></section>`;
  clearAttachment();
  input.value = '';
  input.focus();
  syncSend();
...
document.addEventListener('click', event => {
  ...
  const actions = {
    ...
    restore: () => {
      ...
      if (!previousConversation && theme === 'mesa') {
    ...
    suggestion: () => openSheet('Comece pelo que você tem', `<p>Uma refeição possível começa com a sua rotina. A Mabi pode ajudar a transformar ingredientes disponíveis em uma ideia culinária.</p><img src="assets/${theme}.svg" alt="Ilustração de refeição" style="display:block;width:150px;height:145px;margin:auto"><h3>Abra a geladeira, sem pressa.</h3><p>Escolha os ingredientes que você gostaria de usar e conte quanto tempo tem.</p><button class="primary" data-prompt="Quero uma ideia com o que tenho em casa.">Pensar junto com a Mabi ${icon('arrow')}</button>`),
